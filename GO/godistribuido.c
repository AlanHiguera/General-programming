        ...
        if(elemento == 0 && esta_en_la_lista(regiones, i) == 0){
            ...
            while(hay_elementos_sin_marca(regiones)==1){//mientras haya celdas sin marcar
                ...
                if(0<=(posicion_sin_marca - dim) && (posicion_sin_marca-dim)<=(dim*dim - 1)){//Seccion condicional para visitar las celdas adyacentes
                    ...
                    printf("p->valor: %d\n", aux);
                    if(aux==0 && esta_en_la_lista(regiones, posicion_sin_marca-dim)==0){//vecino de arriba
                ...
                if(0<=(posicion_sin_marca+dim) && posicion_sin_marca+dim<=(dim*dim-1)){//vecino de fila de abajo
                    aux = enesimo_elemento(p, posicion_sin_marca+dim);
                 ...
                 if(0<=(posicion_sin_marca + 1) && (posicion_sin_marca +1)/dim==(posicion_sin_marca)/dim && (posicion_sin_marca + 1)<=(dim*dim-1)){//nos cuidamos de los limites buscando el vecino derecho
                    aux = enesimo_elemento(p, posicion_sin_marca+1);
                    if(aux==0 && esta_en_la_lista(regiones, posicion_sin_marca+1)==0){
                        printf("p->valor: %d\n", aux);
                        regiones = insertar(regiones, posicion_sin_marca + 1, '0');
                    }
                    if(aux==1){
                        cambiar_marca(regiones, posicion_sin_marca, '1');
                        yasecambio = 1;
                 ...
                 if(0<=(posicion_sin_marca -1) && (posicion_sin_marca -1)/dim==(posicion_sin_marca)/dim && (posicion_sin_marca -1)<=(dim*dim-1)){//vecino izquierdo
                    aux = enesimo_elemento(p, posicion_sin_marca-1);
                    if(aux==0 && esta_en_la_lista(regiones, posicion_sin_marca-1)==0){
                        printf("p->valor: %d\n", aux);
                        regiones = insertar(regiones, posicion_sin_marca -1, '0');
            ...
            regiones = insertar(regiones, -1, '#'); //# representa el fin de una sublista asi separo las regiones
        }
    }
...
void procesar_region(int dim, unsigned int k){//Funcion para recibir la informacion de las celdas
    int rank, size;
    MPI_Status status;
    unsigned int celda;

    MPI_Comm_size(MPI_COMM_WORLD, &size);//Obtenemos el tamaño y el rank localmente
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    printf("Proceso [%d] espera mensajes\n", rank);
    for (int i = 0; i < (dim*dim)/k; i = i + 1){//Cantidad de mensajes que tiene que recibir cada nodo (dim*dim)/k
        MPI_Recv(&celda, 1, MPI_UNSIGNED, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
        printf("Proceso [%d] recibió: %u desde el proceso [%d]\n", rank, celda, status.MPI_SOURCE);
    }
}
...
int main(int argc, char *argv[]){
    unsigned char modo;
    unsigned char modo_particion;
    unsigned int k;
    unsigned int vencedor;
    unsigned int contador;
    unsigned int contador_interno;
    char elemento;
    unsigned int elemento2;
    unsigned char aux[1];
    unsigned int fila = -1;
    unsigned int i = 0;
    char processor_name[MPI_MAX_PROCESSOR_NAME];
    int me, allwe, whoami, dim;
    contador = 0;
    contador_interno = 0;
    ...
    MPI_Barrier(MPI_COMM_WORLD);//Sincronizamos los procesos
    if (whoami == 0){
        printf("MASTER: %s\n", processor_name);
        scanf("%d", &dim);
        char celda[dim];//arreglo estatico para poder guardar los datos
        k = allwe;
        if (strcmp(argv[2],"-S") == 0){//guardar input de consola
            modo = 0;
        }
        ...
        printf("k: %d\n", k);
        if (dim % k == 0){
            while (contador < dim*dim){//Enviamos las celdas a los procesos
                if(contador/dim != fila){//se lee la fila por el tipo de dato
                    scanf("%s", celda);
                    fila = fila +1;
                }
                elemento = celda[i]-'0';
                elemento2 = (unsigned int)elemento;
                MPI_Send(&elemento2, 1, MPI_UNSIGNED, contador_interno, 0, MPI_COMM_WORLD);//enviamos segun el contador_interno que se fija en cada cuanto tenemos que cambiar el proceso de destino.
                contador = contador + 1;
                if(i==dim-1){//resetamos i cuando llegamos al final de la fila
                    i = 0;
                }
                else{
                    i = i + 1;
                }
                if(contador_interno == dim-1){//reseteamos el destino ya que solo hay k procesos
                    contador_interno = 0;
                }
                else if(i%k==0){//cambiamos el proceso de destino cada k veces
                    contador_interno = contador_interno + 1;
                }
            }
        }
    }
    procesar_region(dim, k);//Procesamos región en todos los nodos
    fflush(stdout);
    MPI_Finalize();
    return 0;
}